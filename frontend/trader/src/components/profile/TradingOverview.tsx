    ...
    <div className="relative w-full overflow-hidden rounded-xl border border-border-primary bg-bg-secondary p-3">
      <div className="flex flex-wrap items-center gap-3 mb-2 text-[10px] text-text-tertiary">
        <span className="flex items-center gap-1">
          <span className="w-2 h-2 rounded-full bg-[#00d048]" /> Strong profit
        </span>
        <span className="flex items-center gap-1">
      ...
      >
        ...
        <defs>
          <linearGradient id="eqFill" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor="#00d048" stopOpacity="0.25" />
            <stop offset="100%" stopColor="#00d048" stopOpacity="0" />
          </linearGradient>
        </defs>
        ...
        {pathD ? (
          <path
            ...
            stroke={NEON}
            strokeWidth={2}
            className="drop-shadow-[0_0_8px_rgba(0,208,72,0.35)]"
          />
        ) : null}
        {hover ? (
          <>
        ...
        >
          <div className="p-3 md:p-4 border-b border-border-primary flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              <Calendar className="w-5 h-5 text-[#00d048]" />
              ...
              {(
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    calView === t.id ? 'bg-[#00d048] text-text-inverse' : 'bg-bg-secondary text-text-tertiary hover:text-text-primary',
                  )}
                >
                  {t.label}
                </button>
          ...
          <div className="px-3 py-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px] border-b border-border-primary bg-bg-secondary/40">
            <span className="text-[#00d048] font-semibold">Monthly P&L {fmtCompactSigned(s.monthlyPnlUsd)}</span>
            ...
            <span className="ml-auto text-text-tertiary">
              <span className="text-[#00d048]">{s.wins}W</span> <span className="text-red-400">{s.losses}L</span>
            </span>
          </div>
                  ...
                  return (
                    <div
                      ...
                      className={clsx(
                        ...
                        inMonth && isWin && 'border-[#00d048]/50 bg-[#00d048]/10',
                          ...
                          {showUsd ? (
                            <span
                              className={clsx(
                                ...
                                cell.pnlUsd! >= 0 ? 'text-[#00d048]' : 'text-red-400',
                              )}
                            >
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl p-4 border" style={{ backgroundColor: CARD, borderColor: BORDER }}>
            <div className="flex items-center gap-2 text-sm font-semibold mb-3">
              <Target className="w-4 h-4 text-[#00d048]" />
              Trade win %
            </div>
            <p className={clsx('text-3xl font-bold', d.stats.tradeWinPct >= 50 ? 'text-[#00d048]' : 'text-red-400')}>
              {d.stats.tradeWinPct.toFixed(1)}%
            </p>
            <div className="h-2 rounded-full bg-red-500/40 mt-3 overflow-hidden flex">
              <div
                className="h-full bg-[#00d048]"
                style={{ width: `${Math.min(100, d.stats.tradeWinPct)}%` }}
              />
            </div>
            <div className="flex justify-between text-[11px] mt-2">
              <span className="text-[#00d048]">{j.wins} won</span>
              <span className="text-red-400">{j.losses} lost</span>
            </div>
          </div>
          <div className="rounded-xl p-4 border" style={{ backgroundColor: CARD, borderColor: BORDER }}>
            <div className="flex items-center gap-2 text-sm font-semibold mb-3">
              <BarChart3 className="w-4 h-4 text-[#00d048]" />
              Performance
            </div>
            <ul className="space-y-2 text-sm">
              {[
                ['Profit factor', d.stats.profitFactor.toFixed(2), 'text-[#00d048]'],
                ['Avg win', fmtUsd(d.stats.avgWinUsd), 'text-[#00d048]'],
                ['Avg loss', fmtUsd(-d.stats.avgLossUsd), 'text-red-400'],
                ['Period P&L', fmtCompactSigned(d.stats.periodPnlUsd), 'text-[#00d048]'],
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 rounded-xl p-4 border" style={{ backgroundColor: CARD, borderColor: BORDER }}>
          <div className="flex items-center gap-2 mb-3">
            <LineChart className="w-5 h-5 text-[#00d048]" />
        ...
        <div className="space-y-3">
          <div className="rounded-xl p-4 border" style={{ backgroundColor: CARD, borderColor: BORDER }}>
            ...
            <ul className="space-y-2.5 text-sm">
              {[
                ['Risk–reward', d.stats.riskReward, 'text-[#00d048]'],
                ['Expectancy', `${d.stats.expectancyUsd >= 0 ? '+' : ''}${fmtUsd(d.stats.expectancyUsd)}`, d.stats.expectancyUsd >= 0 ? 'text-[#00d048]' : 'text-red-400'],
                ['Best streak', d.stats.bestStreak, 'text-[#00d048]'],
                ['Worst streak', d.stats.worstStreak, 'text-red-400'],
                ['Best trade', fmtUsd(d.stats.bestTradeUsd), 'text-[#00d048]'],
