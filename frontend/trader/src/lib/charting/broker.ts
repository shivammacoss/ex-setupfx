...
export function createBroker(host: any): any {
    ...
    try {
      ...
      for (const pos of positions) {
        ...
        _host?.positionUpdate({
          ...
          ...(cp ? { last: cp } : {}),
          // Brackets — TV draws SL/TP lines on chart natively when present
          ...(pos.stop_loss ? { stopLoss: pos.stop_loss } : {}),
          ...(pos.take_profit ? { takeProfit: pos.take_profit } : {}),
        });
      }
    } catch {}
  }, 3000);
  ...
  return {
    ...
    /* ─── Edit position brackets (drag SL/TP on chart) ─── */
    async editPositionBrackets(positionId: string, modifiedBrackets: any): Promise<void> {
