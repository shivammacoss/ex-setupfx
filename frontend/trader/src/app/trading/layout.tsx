...
export default function TradingLayout({ children }: { children: React.ReactNode }) {
  ...
  // Routes that already render their own DashboardShell (Exness-style chrome).
  // We must NOT wrap them with the legacy TopBar/extra layout, otherwise the
  // user sees two headers stacked on top of each other.
  const usesOwnShell =
    pathname?.startsWith('/trading/performance') ||
    pathname?.startsWith('/trading/history') ||
    pathname?.startsWith('/trading/open-account');
  const userTheme = useUIStore((s) => s.theme);

  // Pages with their own DashboardShell: render children directly inside
  // TradingSession (keeps WebSocket/account bootstrap) but skip the wrapper.
  if (usesOwnShell) {
    return (
