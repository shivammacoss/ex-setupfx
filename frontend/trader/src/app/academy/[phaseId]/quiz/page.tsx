  ...
  return (
    <DashboardShell>
      <div className="page-main max-w-6xl mx-auto w-full pb-8">
        ...
        <div className="bg-bg-secondary border border-[#00d048]/20 rounded-xl p-6 mb-6">
          ...
          <div className="flex items-center gap-3 mb-6">
            <div className="flex-1 h-2 bg-bg-tertiary rounded-full overflow-hidden">
              <div
                className="h-full bg-[#00d048] rounded-full transition-all"
          ...
          <div className="mb-6">
            <p className="text-sm text-text-primary font-medium mb-4">
              <span className="text-[#00d048] font-bold mr-2">{String(currentQuestion + 1).padStart(2, '0')}</span>
              {question.question}
            </p>
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      ...
                      isSelected
                        ? 'border-[#00d048] bg-[#00d048]/5 text-text-primary'
                        : 'border-border-glass bg-bg-tertiary text-text-secondary hover:border-text-tertiary hover:text-text-primary',
                    )}
                  >
                    <span className={clsx('text-xs font-medium', isSelected ? 'text-[#00d048]' : 'text-text-tertiary')}>
                      {letter}
                    </span>
                    {option}
                  </button>
          ...
          <div className="flex items-center justify-between">
            ...
            {currentQuestion < quiz.questions.length - 1 ? (
              <button
                type="button"
                onClick={() => setCurrentQuestion((p) => p + 1)}
                className="flex items-center gap-1 text-sm text-[#00d048] hover:underline"
              >
                NEXT
                <ArrowRight size={14} />
              </button>
        ...
        <button
          ...
          className="px-6 py-3 bg-[#00d048]/20 text-[#00d048] font-semibold rounded-lg text-sm uppercase tracking-wider disabled:opacity-40 disabled:cursor-not-allowed hover:bg-[#00d048]/30 transition-colors"
        >
          SUBMIT ANSWERS
        </button>
