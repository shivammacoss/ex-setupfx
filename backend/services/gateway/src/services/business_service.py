...
async def _generate_unique_ref_code(db: AsyncSession) -> str:
    """6-char uppercase alphanumeric, retry until unique."""
...
async def _ensure_ib_profile(user_id: UUID, db: AsyncSession) -> IBProfile:
    """Auto-vivify an IBProfile for the user — every trader is an IB by default,
    no application/approval required."""
    res = await db.execute(select(IBProfile).where(IBProfile.user_id == user_id))
    profile = res.scalar_one_or_none()
    if profile:
        return profile
    code = await _generate_unique_ref_code(db)
    profile = IBProfile(
        user_id=user_id,
        referral_code=code,
        level=1,
...
async def ib_status(user_id: UUID, db: AsyncSession) -> dict:
    """Auto-create IB profile if missing — every trader is an IB by default."""
    profile = await _ensure_ib_profile(user_id, db)
    return {
        "is_ib": True,
        "referral_code": profile.referral_code,
        "level": profile.level,
        "total_earned": float(profile.total_earned or 0),
        "pending_payout": float(profile.pending_payout or 0),
...
async def apply_ib(user_id: UUID, application_data: dict | None, db: AsyncSession) -> dict:
    """Legacy endpoint — IB is now auto-active. Returns the existing/created profile."""
    profile = await _ensure_ib_profile(user_id, db)
    return {
        "id": str(profile.id),
        "status": "approved",
        "referral_code": profile.referral_code,
        "message": "IB profile active",
    }
