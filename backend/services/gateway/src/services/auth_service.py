...
async def google_login(
    ...
    """Verify a Google ID token (from frontend Google Sign-In), find or create the user,
    and issue session cookies. No password required — auth is via Google's token."""
    import httpx

    rate_limit_http(request, "google-login", 30, 60.0)

    st = get_settings()
    if not st.GOOGLE_CLIENT_ID:
        raise AuthServiceError("Google login is not configured", 501)
    ...
    try:
        ...
        raise AuthServiceError("Failed to verify Google token", 502)

    # Validate audience matches our client ID
    aud = google_data.get("aud", "")
    if aud != st.GOOGLE_CLIENT_ID:
        raise AuthServiceError("Google token audience mismatch", 401)
    ...
    if not google_id or not email:
        raise AuthServiceError("Invalid Google account data", 400)
    if str(email_verified).lower() != "true":
        raise AuthServiceError("Google email is not verified", 400)
    ...
    if not user:
        ...
        if user:
            ...
            # Create default trading accounts (same as register_user)
            ...
            db.add(TradingAccount(
                ...
                leverage=default_leverage, currency="USD", is_demo=True,
            ))
            await db.flush()
    ...
    return await issue_auth_json_response(user, request, db, user_audit_action="GOOGLE_LOGIN")
