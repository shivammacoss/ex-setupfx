...
class AnalystView(Base):
    """TA-engine or external-feed generated short-term trading view shown on /analytics/views."""
    __tablename__ = "analyst_views"

    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    symbol = Column(String(20), nullable=False, index=True)
    category = Column(String(20), nullable=False)        # forex|crypto|stocks|indices|commodities
    timeframe = Column(String(10), nullable=False)       # 5m|30m|1h|4h|1d|1w
    direction = Column(String(8), nullable=False)        # up|down|neutral
    expected_pips_min = Column(Numeric(12, 2), nullable=False, default=0)
    expected_pips_max = Column(Numeric(12, 2), nullable=False, default=0)
    target_price = Column(Numeric(18, 8), nullable=False)
    pivot_price = Column(Numeric(18, 8), nullable=False)
    notes = Column(Text)
    source = Column(String(20), nullable=False, default="ta_engine")
